...
static void showRectangles(Bmp &bmp) {
    bmp.drawRectangle(0, 0, bmp.getWidth() - 1, bmp.getHeight() - 1, Color::cyan());
...
static void showLines(Bmp &bmp) {
    int xCenter = bmp.getWidth() / 2;
    int yCenter = bmp.getHeight() / 2;
...
string datagen(int d) {
    ...
    */
    ...
    if (!image.write(fileName, errMsg)) {
        ...
        cout << "Successfully wrote input file: [" << fileName << "]" << endl;
    }
