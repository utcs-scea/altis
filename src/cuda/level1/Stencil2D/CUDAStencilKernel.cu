...
__device__ int ToGlobalRow(int gidRow, int lszRow, int lidRow) {
  return gidRow * lszRow + lidRow;
}

__device__ int ToGlobalCol(int gidCol, int lszCol, int lidCol) {
...
template <class T>
__global__ void StencilKernel(T *data, T *newData, int alignment,
                              int nStripItems, T wCenter, T wCardinal,
                              T wDiagonal) {
  ...
  for (int i = 0; i < lszRow; i++) {
    ...
    newData[ToFlatIdx(gRow + i, gCol, gRowWidth)] =
...
template <class T>
void CUDAStencil<T>::operator()(Matrix2D<T> &mtx, unsigned int nIters) {
  ...
  cudaMemcpy2D(
      newData +
          (mtx.GetNumRows() - 1) * mtx.GetNumPaddedColumns(), // destination
      rowExtent, // destination pitch
  ...
  for (unsigned int iter = 0; iter < nIters; iter++) {
    this->DoPreIterationWork(currData, newData, mtx, iter);

    // do the stencil operation
    StencilKernel<<<dimGrid, dimBlock, localDataSize>>>(
        currData, newData, mtx.GetPad(), lRows, this->wCenter, this->wCardinal,
        this->wDiagonal);
