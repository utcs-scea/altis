...
template <class T>
Stencil<T> *CUDAStencilFactory<T>::BuildStencil(const OptionParser &options) {
  ...
  this->ExtractOptions(options, wCenter, wCardinal, wDiagonal, lRows, lCols,
                       devs);
  ...
  return new CUDAStencil<T>(wCenter, wCardinal, wDiagonal, lRows, lCols,
                            chosenDevice);
}
