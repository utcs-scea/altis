...
__global__ void radixSortBlocks(const uint nbits, const uint startbit,
                                uint4 *keysOut, uint4 *valuesOut, uint4 *keysIn,
                                uint4 *valuesIn) {
...
__global__ void reorderData(uint startbit, uint *outKeys, uint *outValues,
                            uint2 *keys, uint2 *values, uint *blockOffsets,
                            uint *offsets, uint *sizes, uint totalBlocks) {
  ...
  globalOffset =
      sOffsets[radix] + threadIdx.x + GROUP_SIZE - sBlockOffsets[radix];
...
__global__ void scan(uint *g_odata, uint *g_idata, uint *g_blockSums,
  ...
  if (fullBlock || i + 3 < n) {
    ...
    res = threadScanT.w;
  } else { // if not, read individual uints, scan & store in lmem
